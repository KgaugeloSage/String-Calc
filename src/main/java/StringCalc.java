...
public class StringCalc {
    ...
    public int add(String string) throws Exception { // takes a string as a parameter and calculates the sum of the integers.
    ...
    private void errors(String string) throws Exception { // check if a string is in a required format and throw exceptions

        Matcher matcher = correctFormat.matcher(this.string);

        if (negatives.length > 0) {
            throw new IllegalArgumentException("ERROR: negatives not allowed " + Arrays.toString(negatives).replace("[", "").replace("]", ""));
        }
